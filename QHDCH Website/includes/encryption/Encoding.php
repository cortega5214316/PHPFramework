<?php
	...
	include_once("BadFormatException.php");

	final class Encoding
		{
			const CHECKSUM_BYTE_SIZE     = 32;
			const CHECKSUM_BYTE_SIZE_64  = 64;
			const CHECKSUM_HASH_ALGO     = 'sha256';
			const CHECKSUM_HASH_ALGO_64  = 'sha512';
			 ...
			 * @return string
			 */
			public static function saveBytesToChecksummedAsciiSafeString64($header, $bytes)
				{
					// Headers must be a constant length to prevent one type's header from
					// being a prefix of another type's header, leading to ambiguity.
					if (Core::ourStrlen($header) !== self::SERIALIZE_HEADER_BYTES) 
			 ...
			 * @return string
			 */
			public static function loadBytesFromChecksummedAsciiSafeString64($expected_header, $string)
				{
					// Headers must be a constant length to prevent one type's header from
					// being a prefix of another type's header, leading to ambiguity.
					if (Core::ourStrlen($expected_header) !== self::SERIALIZE_HEADER_BYTES) 
					...
					/* Make sure we have enough bytes to get the version header and checksum. */
					if (Core::ourStrlen($bytes) < self::SERIALIZE_HEADER_BYTES + self::CHECKSUM_BYTE_SIZE_64) 
						{
							throw new BadFormatException(
																'Encoded data is shorter than expected.'
															);
					...
					$checked_bytes = Core::ourSubstr(
														$bytes,
														0,
														Core::ourStrlen($bytes) - self::CHECKSUM_BYTE_SIZE_64
													);
					...
					$checksum_a = Core::ourSubstr(
													$bytes,
													Core::ourStrlen($bytes) - self::CHECKSUM_BYTE_SIZE_64,
													self::CHECKSUM_BYTE_SIZE_64
												);

					/* Re-compute the checksum. */
					$checksum_b = hash(self::CHECKSUM_HASH_ALGO_64, $checked_bytes, true);
					...
					return Core::ourSubstr(
											$bytes,
											self::SERIALIZE_HEADER_BYTES,
											Core::ourStrlen($bytes) - self::SERIALIZE_HEADER_BYTES - self::CHECKSUM_BYTE_SIZE_64
										);
				}																																								
		}
