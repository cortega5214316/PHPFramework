	...
	final class Core
		{
			...
			const SALT_BYTE_SIZE_64                 = 64;
			const MAC_BYTE_SIZE_64                  = 64;
			const HASH_FUNCTION_NAME                = 'sha256';
			const HASH_FUNCTION_NAME_512            = 'sha512';
			 ...
			 * @return bool
			 */
			public static function hashEquals64($expected, $given)
				{
					static $native = null;
					if ($native === null) 
						{
					...
					$blind           = Core::secureRandom(64);
					$message_compare = hash_hmac(Core::HASH_FUNCTION_NAME_512, $given, $blind);
					$correct_compare = hash_hmac(Core::HASH_FUNCTION_NAME_512, $expected, $blind);
					return $correct_compare === $message_compare;
				}
			...
			public static function ensureFunctionExists($name)
				{
					if (!function_exists($name)) 
						{
							throw new EnvironmentIsBrokenException('Function does not exist'. $name);
					...
					if ($exists) 
						{
							...
							if ($length === false) 
								{
									throw new EnvironmentIsBrokenException('wrong string length');
								}
							return $length;
						} 
					else 
